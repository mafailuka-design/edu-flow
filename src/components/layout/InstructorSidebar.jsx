...
export default function InstructorSidebar({ onCloseMobile = () => {} }) {
  ...
  const navItems = [
    ...
    { to: '/instructor/courses', label: 'My Courses', icon: BookOpen, badge: instructorCourses.length },
    { to: '/instructor/create-course', label: 'Create Course', icon: PlusCircle },
    { to: '/courses', label: 'Public Catalog', icon: Compass },
    ...
    <aside className="w-64 bg-white border-r border-slate-200/80 flex flex-col justify-between h-full min-h-[calc(100vh-4rem)]">
      ...
      <div className="p-4 space-y-6">
        ...
        <div className="p-3.5 bg-purple-50/70 border border-purple-100 rounded-2xl flex items-center gap-3">
          <Avatar src={user?.avatar} name={user?.name || "Instructor"} size="md" status="online" />
          <div className="min-w-0 flex-1">
            <p className="text-sm font-bold text-slate-900 truncate">{user?.name || "Dr. Sarah Jenkins"}</p>
            <div className="flex items-center gap-1.5 mt-0.5">
              <Badge variant="purple" size="xs">Instructor</Badge>
      ...
      <div className="p-4 border-t border-slate-100 space-y-2 bg-slate-50/40">
        <button
          ...
          className="w-full flex items-center justify-between px-3.5 py-2.5 text-xs font-semibold text-brand-700 bg-brand-50 hover:bg-brand-100 rounded-xl border border-brand-200 transition-colors"
        >
          <span className="flex items-center gap-2">
            <Repeat className="w-3.5 h-3.5" />
            Switch to Student View
          </span>
          <span className="text-[10px] bg-white px-1.5 py-0.5 rounded font-bold border border-brand-200">
            Demo
          </span>
        </button>
